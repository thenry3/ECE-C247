...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    layer_1 = np.dot(X, W1.T) + b1
    out1 = relu(layer_1)
    
    second_layer = np.dot(out1, W2.T) + b2
    scores = second_layer
    ...
    loss = -scores[range(N), y] + np.log(sum_scores_exp)
    ...
    grad = scores_exp / sum_scores_exp.reshape((-1, 1))
    grad[range(N), y] -= 1
    grad /= N
    
    grads["W2"] = np.dot(grad.T, out1) + reg * W2
    grads["b2"] = np.sum(grad.T, axis=1)
    
    dldc = np.dot(grad, W2)
    dldrelu = (np.dot(X, W1.T) > 0) * dldc
    
    grads["W1"] = np.dot(dldrelu.T, X) + reg * W1
    grads["b1"] = np.sum(dldrelu.T, axis=1)
  ...
  def predict(self, X):
    ...
    out = np.dot(X, self.params["W1"].T) + self.params["b1"]
    out = np.maximum(out, 0)
    
    out = np.dot(out, self.params["W2"].T) + self.params["b2"]
